...
import { FiPlus, FiEdit2, FiTrash2, FiCheck, FiImage, FiInfo } from 'react-icons/fi';
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Banner Yönetimi</h1>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Görsel URL * <span className="text-gray-400 font-normal">({spec.width}x{spec.height})</span>
              </label>
              <input
                type="url"
                value={form.imageUrl}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Link URL <span className="text-gray-400 font-normal">(opsiyonel)</span></label>
              <input
                type="text"
                value={form.linkUrl}
            ...
            <div className="flex items-end">
              {form.imageUrl && (
                <div className="w-full">
                  ...
                  <img src={form.imageUrl} alt="Preview" className="h-16 rounded-lg object-cover border border-gray-200" onError={(e) => (e.currentTarget.style.display = 'none')} />
                </div>
              )}
            </div>
          ...
          return (
            <div key={type}>
              <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-2">
                {spec.label}
                <span className="text-xs font-normal text-gray-400">({spec.width}x{spec.height})</span>
                <span className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full text-xs">{typeBanners.length}</span>
              </h3>
              <div className="grid gap-3">
                {typeBanners.map((b) => (
                  <div key={b.id} className={`bg-white rounded-xl border ${b.active ? 'border-gray-200' : 'border-gray-100 opacity-60'} p-4 flex items-center gap-4`}>
                    <img
                      src={b.imageUrl}
                      alt={b.title}
                      className={`${type === 'HERO' ? 'w-40 h-14' : 'w-40 h-8'} object-cover rounded-lg border border-gray-100`}
                    />
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-gray-900 text-sm truncate">{b.title}</p>
                      {b.linkUrl && <p className="text-xs text-gray-400 truncate">{b.linkUrl}</p>}
                    </div>
                    <span className="text-xs text-gray-400 font-mono">#{b.sortOrder}</span>
                    <button
                    ...
                    >
                      {b.active ? <><FiCheck size={12} className="inline mr-1" />Aktif</> : 'Pasif'}
                    </button>
                    <button onClick={() => handleEdit(b)} className="p-2 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition">
                      <FiEdit2 size={16} />
                    </button>
                    <button onClick={() => handleDelete(b.id)} className="p-2 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition">
                      <FiTrash2 size={16} />
                    </button>
                  </div>
                ))}
